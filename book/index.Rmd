...
# check if species cache data is available
...
# check if grid cache data is available
if (inherits(cache.grid.PLY, 'SpatialPolygons')) {
  # check if grid data has changed
  if (!identical(grid.PLY, cache.grid.PLY)) {
    use.cache[['make_species_maps']] <- FALSE
  }
}

## save cache for debugging
write.table(tidyr::gather(as.data.frame(use.cache),method,value), file='logs/cache_usage.csv', row.names=FALSE, sep=',')
